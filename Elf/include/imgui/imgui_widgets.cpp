...
void ImGui::ProgressBar2(float &fraction, const ImVec2& size_arg, const char* overlay, float dis, float speed)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	bb.Expand(ImVec2(-style.FrameBorderSize, -style.FrameBorderSize));
	const ImVec2 fill_br = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction), bb.Max.y);
	float fractionFront = fraction<dis ? 0.0f : (fraction - dis);
	float fractionAfter = fraction<dis ? (fraction) : ((fraction) >= 1.0f ? 1.0f : (fraction));
	RenderRectFilledRangeH(window->DrawList, bb, GetColorU32(ImGuiCol_PlotHistogram), fractionFront, fractionAfter, style.FrameRounding);

	if (((fractionAfter - fractionFront) <= speed) && (fractionAfter == 1.0f))
	{
		fraction = 0;
	}
